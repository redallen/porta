...
class ApiDocs::Service < ApplicationRecord
  ...
  validate :service_belongs_to_account, if: -> { service_id.present? || owner_type == 'Service' }
  ...
  scope :deprecated_service_accessible, -> { joining { service.outer }.where.has { service.state != ::Service::DELETE_STATE } }
